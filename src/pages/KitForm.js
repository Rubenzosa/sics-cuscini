...
export default function KitForm({ kits, reload }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!existing) {
      const duplicate = tuttiKits.find(k => Number(k.numero) === Number(form.numero));
      if (duplicate) {
        alert(`Numero kit ${form.numero} già in uso (Kit ${duplicate.numero} — ${duplicate.nome}). Scegli un numero diverso.`);
        setSaving(false);
        return;
      }
    }
